...
download_redirects:
	@mkdir -p $(OUT_DIR)
	@curl -s $(WD_ENDPOINT) -H "Accept: text/tab-separated-values" --data-urlencode query="PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX owl: <http://www.w3.org/2002/07/owl#> SELECT ?ent (GROUP_CONCAT(DISTINCT ?redir; SEPARATOR = \"; \") AS ?redirs) WHERE { ?redir owl:sameAs ?ent . FILTER(REGEX(STR(?ent), \"entity/Q\\\\d+\")) } GROUP BY ?ent" \
	--data-urlencode access-token=$(WD_ACCESS_TOKEN) --data-urlencode timeout=$(TIMEOUT) \
	> $(OUT_DIR)/wikidata-entity-redirects.tsv
	@curl -s $(DB_ENDPOINT) -H "Accept: text/tab-separated-values" --data-urlencode query="PREFIX dbo: <http://dbpedia.org/ontology/> PREFIX dbr: <http://dbpedia.org/resource/> SELECT ?target (GROUP_CONCAT(DISTINCT ?source; SEPARATOR = \"; \") as ?sources) WHERE { ?source dbo:wikiPageRedirects ?target } GROUP BY ?target" \
	--data-urlencode access-token=$(DB_ACCESS_TOKEN) --data-urlencode timeout=$(TIMEOUT) \
