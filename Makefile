...
download_entities:
	@mkdir -p $(OUT_DIR)
	@curl -s https://qlever.cs.uni-freiburg.de/api/wikidata -H "Accept: text/tab-separated-values" -H "Content-type: application/sparql-query" --data "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX wikibase: <http://wikiba.se/ontology#> PREFIX schema: <http://schema.org/> PREFIX wdt: <http://www.wikidata.org/prop/direct/> PREFIX skos: <http://www.w3.org/2004/02/skos/core#> PREFIX wd: <http://www.wikidata.org/entity/> SELECT ?entity ?entity_name ?entity_description (MAX(?sitelinks) AS ?links) (GROUP_CONCAT(DISTINCT ?alias; SEPARATOR = \"; \") AS ?aliases) WHERE { { ?entity wdt:P18 ?pic } UNION { ?article schema:about ?entity . ?article schema:isPartOf ?wiki . FILTER(REGEX(STR(?wiki), \"^https?://.*.wikipedia.org\")) } ?entity ^schema:about/wikibase:sitelinks ?sitelinks . ?entity rdfs:label ?entity_name . FILTER (LANG(?entity_name) = \"en\") . OPTIONAL { ?entity schema:description ?entity_description . FILTER (LANG(?entity_description) = \"en\") } OPTIONAL { ?entity skos:altLabel ?alias . FILTER (LANG(?alias) = \"en\") } } GROUP BY ?entity ?entity_name ?entity_description ORDER BY DESC(?links)" \
